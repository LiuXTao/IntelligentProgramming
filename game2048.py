...
class Node(object):
    def __init__(self, state):
        self.parent = None  # 记录当前结点的父结点
        self.children = {}  # 记录当前结点采取行动之后的可能子结点
        self.n_visits = 1  # 记录当前结点访问次数
        self.q_value = 0.0  # 记录当前结点的Q值
        self.state = state  # 每一个结点对应一个状态state

class MCTS(object):
    def __init__(self, env, c, gamma, depth, loop_times):
        self.T = {}
        self.gameEnv = env
        self.depth = depth
        self.c = c
        ...
        self.action_list = [0, 1, 2, 3]  # up down left right

    def mcts_search(self, state):
        for i in range(self.loop_times):
            self.simulate(state, self.depth)
        cur_node = self.T[str(state)]
        return self.find_best_action(cur_node=cur_node, with_c=False)

    def simulate(self, state, d):
        if d == 0:
            return 0
        if str(state) not in self.T.keys():
            cur_node = Node(copy.deepcopy(state))
            cur_node.n_visits = 1
            cur_node.q_value = 0
            for action in self.action_list:
                next_node = Node(copy.deepcopy(state))
                next_node.n_visits = 1
                next_node.q_value = 0
                next_node.parent = cur_node
                cur_node.children[action] = next_node
                cur_node.n_visits += 1
            self.T[str(state)] = cur_node
            return self.rollout_evaluation(state, d)
        else:
            cur_node = self.T[str(state)]

        action = self.find_best_action(cur_node=cur_node, with_c=True)

        self.gameEnv.state = copy.deepcopy(state)
        next_state, r, done, _ = self.gameEnv.step(action)
        q = r + self.gamma * self.simulate(next_state, d-1)
        cur_node.children[action].n_visits += 1
        cur_node.n_visits += 1
        cur_node.children[action].q_value += ((q - cur_node.children[action].q_value)/cur_node.children[action].n_visits)
        return q

    def rollout_evaluation(self, state, d):
        if d == 0:
            return 0
        ...
        return r + self.gamma * self.rollout_evaluation(next_state, d-1)

    def find_best_action(self, cur_node, with_c = False):
        action = 0
        maxQ = -1
        for a in self.action_list:
            q_a = cur_node.children[a].q_value
            if with_c == True:
                q_a += self.c * np.sqrt(np.log(cur_node.n_visits) / cur_node.children[a].n_visits)
            if maxQ <= q_a:
                maxQ = q_a
                action = a
        return action
...
if __name__ == '__main__':
    ...
    print(env.state, test_env.state)

    mcts = MCTS(test_env, c=100, gamma=0.99, depth=10, loop_times=50)
    iters = 0

    while not done:
        # A random agent as example
        # 0, 1, 2, 3 mean up, down, left, right respectively
        action = mcts.mcts_search(env.state)
        print("iteration: ", iters, ", action: ", action)
        iters += 1
        # time.sleep(0.1)
        # 题目要求每秒执行一个动作
        time.sleep(1)
