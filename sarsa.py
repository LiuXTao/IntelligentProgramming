...
class Sarsa:
    ...
    def create_Qtable(self, rows=4, columns=12):
        terminal = columns*rows - 1
        # Qtable 为 [action, states_num]
        # Qtable = np.random.rand(4, columns*rows)
        Qtable = np.zeros((4, terminal+1))
    ...
    # 更新Q值表
    def update_Qtable(self, state, action, reward, next_state_value):
        update_qvalue = self.Qtable[action, state] + self.alpha*(reward + self.gamma * next_state_value - self.Qtable[action, state])
    ...
    def sarsa(self):
        for iters in range(self.play_rounds):
            ...
            while game_over != 1:
                ...
                # 更新Qtable
                self.update_Qtable(state=state, action=action, reward=\
                    reward, next_state_value=self.Qtable[next_action, next_state])
                state, action = next_state, next_action
            self.reward_record.append(reward_sum)
            if iters == self.play_rounds -1:
                print("Agent trained with SARSA after", self.play_rounds, " iteration")
                print(env)
                self.gameEnv = env
    ...
    def visulize_heatmap(self, show_data, method="Sarsa"):
        plt.figure()
    ...
    def visulize_env(self, show_data, method="Sarsa"):
        plt.figure()
